...
function sortShuffle(){
  const countries = data.countries;
  return countries.sort(function(){
...
function filter(list, option) {
   if (option === "all" || option === "1") {
      ...
      return list.filter((c) => c.population >= 100000000);
    } else if (option === "200m") {
      return list.filter((c) => c.population >= 200000000);
    } else if (option === "500m") {
      return list.filter((c) => c.population >= 500000000);
    } else if (option === "1b") {
      return list.filter((c) => c.population >= 1000000000);
    } else {
      return list.filter((c) => c.continent.toLowerCase() === option);
    }
}
